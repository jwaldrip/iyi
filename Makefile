...
# beside `bin/iyi` is 78 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 78 KB answers `require "json"` with "can't find file",
...
# Its prelude is its own, and it is 78 KB: `iyi` installed as `bin/iyi` finds
