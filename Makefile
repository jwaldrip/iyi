...
# beside `bin/iyi` is 538 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 538 KB answers `require "json"` with "can't find file",
...
# Crystal's library requires `std/...`, the directory's 1,857 KB is a second
...
# Its prelude is its own, and it is 538 KB: `iyi` installed as `bin/iyi` finds
