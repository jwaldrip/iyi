...
# beside `bin/iyi` is 536 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 536 KB answers `require "json"` with "can't find file",
...
# Crystal's library requires `std/...`, the directory's 152 KB is a second
...
# Its prelude is its own, and it is 536 KB: `iyi` installed as `bin/iyi` finds
