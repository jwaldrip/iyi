...
# beside `bin/iyi` is 71 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 71 KB answers `require "json"` with "can't find file",
...
# Its prelude is its own, and it is 71 KB: `iyi` installed as `bin/iyi` finds
