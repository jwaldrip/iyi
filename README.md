...
it uses, and iyi's own library is 11,498 lines rather than 8,161. The whole
library is 459 KB on disk beside the binary.
...
The tarball is relocatable and carries both libraries: iyi's own 459 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 11,498
...
Fifteen programs in [`samples/iyi`](samples/iyi), each documenting a part of
...
`init_order`, `webapp`, `calc`, `derive`, `files` and `formatting`. And three
...
`config` the first that reads text, and they are there because the prelude
...
- **iyi's own library is 11,498 lines, and its IO is `puts`, `print`,
...
| [`samples/iyi`](samples/iyi) | fifteen programs: eleven documenting a part of it, three being a first hour, and `calc`, a language |
...
| [`src/iyi`](src/iyi) | iyi's own library, 11,498 lines. `--crystal` swaps it for Crystal's |
