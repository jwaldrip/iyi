...
it uses, and iyi's own library is 13,878 lines rather than 8,161. The whole
library is 537 KB on disk beside the binary.
...
iyi's own 537 KB prelude, the 152 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 13,878
...
- **iyi's own library is 13,878 lines, and its IO is `puts`, `print`,
...
| [`src/iyi`](src/iyi) | iyi's own library, 13,878 lines. `--crystal` swaps it for Crystal's |
