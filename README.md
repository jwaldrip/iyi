...
it uses, and iyi's own library is 11,623 lines rather than 8,161. The whole
library is 462 KB on disk beside the binary.
...
The tarball is relocatable and carries both libraries: iyi's own 462 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 11,623
...
Sixteen programs in [`samples/iyi`](samples/iyi), each documenting a part of
...
`init_order`, `webapp`, `calc`, `derive`, `files` and `formatting`. And four
...
`config` the first that reads text, `grid` the first with a table, and they
are there because the prelude grows only when a program in this repository
needs something.
...
- **iyi's own library is 11,623 lines, and its IO is `puts`, `print`,
...
| [`samples/iyi`](samples/iyi) | sixteen programs: eleven documenting a part of it, four being a first hour, and `calc`, a language |
...
| [`src/iyi`](src/iyi) | iyi's own library, 11,623 lines. `--crystal` swaps it for Crystal's |
