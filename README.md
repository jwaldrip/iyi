...
it uses, and iyi's own library is 11,425 lines rather than 8,161. The whole
library is 457 KB on disk beside the binary.
...
The tarball is relocatable and carries both libraries: iyi's own 457 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 11,425
...
Fourteen programs in [`samples/iyi`](samples/iyi), each documenting a part of
the design rather than showing off: `hello` (traits and `impl`), `modules`
...
`init_order`, `webapp`, `calc`, `derive`, `files` and `formatting`. And two
that document nothing: `basics` is the seven programs a person writes in
their first half hour, `inventory` the first one with a struct in it, and
they are there because the prelude grows only when a program in this
repository needs something.
...
- **iyi's own library is 11,425 lines, and its IO is `puts`, `print`,
...
| [`samples/iyi`](samples/iyi) | fourteen programs: eleven documenting a part of it, two being a first hour, and `calc`, a language |
...
| [`src/iyi`](src/iyi) | iyi's own library, 11,425 lines. `--crystal` swaps it for Crystal's |
