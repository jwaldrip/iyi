...
The released tarball is 0.5.1, and a build from current source reports the
same.

```console
$ tar -xzf iyi-0.5.1-linux-x86_64.tar.gz -C ~/.local
...
**Is the syntax stable?** No. 0.5.1 is a release of a language that is still
...
language rather than the library — master is 0.5.1, and the parts of SPEC.md
...
- **Artifacts are identified by released version, target and flag set.** Builds
  ...
  commit in its identity and interoperates only with itself. 0.5.1 is
  released, so its artifacts carry the version alone and any 0.5.1 build on
...
`iyi 0.5.1 (built on Crystal 1.22.0-dev)`: the language first, then what it
