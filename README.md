...
it uses, and iyi's own library is 13,850 lines rather than 8,161. The whole
library is 536 KB on disk beside the binary.
...
iyi's own 536 KB prelude, the 152 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 13,850
...
- **iyi's own library is 13,850 lines, and its IO is `puts`, `print`,
...
| [`src/iyi`](src/iyi) | iyi's own library, 13,850 lines. `--crystal` swaps it for Crystal's |
