...
iyi's own 525 KB prelude, the 1,867 KB of `src/std` that `import std/...`
...
| [`src/std`](src/std) | the standard library, 67,998 lines of iyi across 102 modules: collections, text, time, JSON, YAML, XML, TLS 1.3, HTTP/1.1, WebSocket, HTTP/2, QUIC and HTTP/3 among them. Opt-in with `import std/...`, outside the prelude's ceiling |
