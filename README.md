...
it uses, and iyi's own library is 15,968 lines rather than 8,161. The whole
...
iyi's own 635 KB prelude, the 1,274 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 15,968
...
- **iyi's own library is 15,968 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 15,968 lines. `--crystal` swaps it for Crystal's |
