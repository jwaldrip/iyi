...
| [`src/std`](src/std) | the standard library, 43,273 lines of iyi across forty-three modules: TLS 1.3, HTTP/1.1, WebSocket, HTTP/2, QUIC and HTTP/3 among them. `std/http_client` keeps protocol negotiation below one `Client` API. Opt-in with `import std/...`, outside the prelude's ceiling |
