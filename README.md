...
it uses, and iyi's own library is 11,804 lines rather than 8,161. The whole
library is 468 KB on disk beside the binary.
...
The tarball is relocatable and carries both libraries: iyi's own 468 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 11,804
...
Nineteen programs in [`samples/iyi`](samples/iyi), each documenting a part
...
`calc`, `derive`, `files` and `formatting`. And six that document nothing:
...
own, `sessions` the first that forgets an entry, and they are there because
the prelude grows only when a program in this repository needs something.
...
- **iyi's own library is 11,804 lines, and its IO is `puts`, `print`,
...
| [`samples/iyi`](samples/iyi) | nineteen programs: twelve documenting a part of it, six being a first hour, and `calc`, a language |
...
| [`src/iyi`](src/iyi) | iyi's own library, 11,804 lines. `--crystal` swaps it for Crystal's |
