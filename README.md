...
iyi's own 525 KB prelude, the 1,662 KB of `src/std` that `import std/...`
...
| [`src/std`](src/std) | the standard library, 60,046 lines of iyi across ninety-nine modules: collections, text, time, JSON, YAML, XML, TLS 1.3, HTTP/1.1, WebSocket, HTTP/2, QUIC and HTTP/3 among them. Opt-in with `import std/...`, outside the prelude's ceiling |
