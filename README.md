...
current compiler reports `0.4.0`. iyi's own prelude has no IO beyond
...
it uses, and iyi's own library is 3,825 lines rather than 8,161. The whole
library is 127 KB on disk beside the binary.
...
The released tarball is 0.4.0, and a build from current source reports the
same.

```console
$ tar -xzf iyi-0.4.0-linux-x86_64.tar.gz -C ~/.local
$ ~/.local/bin/iyi run ~/.local/share/iyi/samples/hello.iyi
```

The tarball is relocatable and carries both libraries: iyi's own 127 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 3,825
...
**Is the syntax stable?** No. 0.4.0 is a release of a language that is still
...
language rather than the library — master is 0.4.0, and the parts of SPEC.md
marked PROPOSED are the parts that will move under you.

## What is not here

- **iyi's own library is 3,825 lines, and its IO is `puts`, `print`,
...
- **Artifacts are identified by released version, target and flag set.** Builds
  ...
  commit in its identity and interoperates only with itself. 0.4.0 is
  released, so its artifacts carry the version alone and any 0.4.0 build on
...
| [`src/iyi`](src/iyi) | iyi's own library, 3,825 lines. `--crystal` swaps it for Crystal's |
...
`iyi 0.4.0 (built on Crystal 1.22.0-dev)`: the language first, then what it
