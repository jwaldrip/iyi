...
and no more: no sockets, no TLS, no serialisation - `import std/socket` and
`import std/format` are where the first and the format strings live, outside
the prelude a program cannot avoid. Its concurrency — a
...
- **iyi's own library is 13,424 lines, and its IO is `puts`, `print`,
  ...
  `samples/iyi/files.iyi` writes, reads and deletes its file. No sockets and
  no format strings in the prelude: both are `src/std/` modules, and a
  program that imports neither carries neither.
