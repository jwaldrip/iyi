...
it uses, and iyi's own library is 15,935 lines rather than 8,161. The whole
...
iyi's own 628 KB prelude, the 1,270 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 15,935
...
- **iyi's own library is 15,935 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 15,935 lines. `--crystal` swaps it for Crystal's |
