...
it uses, and iyi's own library is 15,926 lines rather than 8,161. The whole
library is 633 KB on disk beside the binary.
...
iyi's own 633 KB prelude, the 1,274 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 15,926
...
- **iyi's own library is 15,926 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 15,926 lines. `--crystal` swaps it for Crystal's |
