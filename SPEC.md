...
| iyi's own prelude | 15,765 lines, of which 3,564 are the library held to the 3,734 ceiling (4,709 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
...
> "No standard library worth the name" is still true of iyi's own 15,765 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 15,765 lines,
primitives included, of which the library is 3,564.** Not a standard library:
...
**3,564 lines** of the 15,765, measured by `bench/doc_numbers.py` as
...
below settles and explains. Opening `src/iyi/` counts 4,709 with the floor
...
| Library | 8,161 lines (3,551 of it core) | 15,765-line own prelude + 38,914 in std |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 15,765-line prelude.
