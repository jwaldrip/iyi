...
describe "Code gen: generic class type" do
  ...
  it "uses big integer as generic type argument (#2353)" do
    run(<<-CODE).to_u64.should eq(2374623294237463578)
      require "prelude"
  ...
  it "doesn't use virtual + in type arguments (#2839)" do
    run(<<-CODE).to_string.should eq("Gen(Foo)")
      class Class
        def name : String
          {{ @type.name.stringify }}
        end
  ...
  it "doesn't use virtual + in type arguments for Tuple (#2839)" do
    run(<<-CODE).to_string.should eq("Tuple(Foo)")
      class Class
        def name : String
          {{ @type.name.stringify }}
        end
  ...
  it "doesn't use virtual + in type arguments for NamedTuple (#2839)" do
    run(<<-CODE).to_string.should eq("NamedTuple(x: Foo)")
      class Class
        def name : String
          {{ @type.name.stringify }}
        end
