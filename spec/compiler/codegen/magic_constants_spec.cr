...
describe "Code gen: magic constants" do
  it "does __LINE__" do
    run(<<-CODE, inject_primitives: false).to_i.should eq(5)
      def foo(x = __LINE__)
        x
      end
  ...
  it "does __FILE__" do
    run(<<-CODE, filename: "/foo/bar/baz.cr").to_string.should eq("/foo/bar/baz.cr")
      def foo(x = __FILE__)
        x
      end
  ...
  it "does __DIR__" do
    run(<<-CODE, filename: "/foo/bar/baz.cr").to_string.should eq("/foo/bar")
      def foo(x = __DIR__)
        x
      end
  ...
  it "does __LINE__ with dispatch" do
    run(<<-CODE, inject_primitives: false).to_i.should eq(10)
  ...
  it "does __LINE__ when specifying one normal default arg" do
    run(<<-CODE, inject_primitives: false).to_i.should eq(27)
      require "primitives"
  ...
  it "does __END_LINE__ with block" do
    run(<<-CODE, inject_primitives: false).to_i.should eq(8)
      def foo(x = __END_LINE__)
        yield
        x
      end
  ...
  it "does __END_LINE__ in macro with block" do
    run(<<-CODE, inject_primitives: false).to_i.should eq(7)
      macro foo(line = __END_LINE__)
        {{line}}
      end
