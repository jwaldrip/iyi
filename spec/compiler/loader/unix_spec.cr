...
describe Iyi::Loader do
  describe ".parse" do
    it "parses directory paths" do
      loader = Iyi::Loader.parse(["-L", "/foo/bar/baz", "--library-path", "qux"], search_paths: [] of String)
      loader.search_paths.should eq ["/foo/bar/baz", "qux"]
    end

    it "prepends directory paths before default search paths" do
      loader = Iyi::Loader.parse(%w(-Lfoo -Lbar), search_paths: %w(baz quux))
      loader.search_paths.should eq %w(foo bar baz quux)
    end

    it "parses static" do
      expect_raises(Iyi::Loader::LoadError, "static libraries are not supported by Crystal's runtime loader") do
        Iyi::Loader.parse(["-static"], search_paths: [] of String)
      end
    end

    it "parses library names" do
      expect_raises(Iyi::Loader::LoadError, "cannot find -lfoobar") do
        Iyi::Loader.parse(["-l", "foobar"], search_paths: [] of String)
      end
      expect_raises(Iyi::Loader::LoadError, "cannot find -lfoobar") do
        Iyi::Loader.parse(["--library", "foobar"], search_paths: [] of String)
      end
    end

    it "parses file paths" do
      exc = expect_raises(Iyi::Loader::LoadError, /no such file|not found|cannot open/i) do
        Iyi::Loader.parse(["foobar.o"], search_paths: [] of String)
      end
      exc.message.should contain File.join(Dir.current, "foobar.o")
      exc = expect_raises(Iyi::Loader::LoadError, /no such file|not found|cannot open/i) do
        Iyi::Loader.parse(["-l", "foo/bar.o"], search_paths: [] of String)
      end
      {% if flag?(:openbsd) %}
  ...
  describe ".read_ld_conf" do
    it "basic" do
      ary = [] of String
      Iyi::Loader.read_ld_conf(ary, compiler_datapath("loader/ld.so/basic.conf"))
      ary.should eq ["foo/bar", "baz/qux"]
    end

    it "with include" do
      ary = [] of String
      Iyi::Loader.read_ld_conf(ary, compiler_datapath("loader/ld.so/include.conf"))
  ...
  describe "dynlib" do
    ...
    describe "#load_file?" do
      it "finds function symbol" do
        loader = Iyi::Loader.new([] of String)
    ...
    describe "#load_library?" do
      it "library name" do
        loader = Iyi::Loader.new([SPEC_CRYSTAL_LOADER_LIB_PATH] of String)
      ...
      it "lookup in order" do
        ...
        help_loader1 = Iyi::Loader.new([SPEC_CRYSTAL_LOADER_LIB_PATH] of String)
        help_loader1.load_library?("foo").should be_true
        foo_address = help_loader1.find_symbol?("foo").should_not be_nil

        help_loader2 = Iyi::Loader.new([SPEC_CRYSTAL_LOADER_LIB_PATH] of String)
        ...
        loader1 = Iyi::Loader.new([SPEC_CRYSTAL_LOADER_LIB_PATH] of String)
