...
describe "Normalize: hash literal" do
  ...
  it "normalizes non-empty with of" do
    assert_expand "{1 => 2, 3 => 4} of Int => Float", <<-CODE
  ...
  it "hoists complex element expressions, hash-like" do
    assert_expand_named "Foo{[1] => 2, 3 => [4]}", <<-CODE
      __temp_1 = [1]
      __temp_2 = [4]
      __temp_3 = Foo.new
      __temp_3[__temp_1] = 2
  ...
  it "hoists complex element expressions, hash-like generic" do
    assert_expand_named "Foo{[1] => 2, 3 => [4]}", <<-CODE, generic: "Foo"
      __temp_1 = [1]
      __temp_2 = [4]
  ...
  it "evaluates key and value expressions in correct order" do
    assert_expand "{foo(1) => foo(2), foo(3) => foo(4), foo(5) => foo(6)}", <<-CODE
      __temp_1 = foo(1)
      __temp_4 = foo(2)
      __temp_2 = foo(3)
      __temp_5 = foo(4)
