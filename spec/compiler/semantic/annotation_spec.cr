...
describe "Semantic: annotation" do
  ...
  describe "#annotation" do
    ...
    it "errors if annotation doesn't exist" do
      assert_error <<-CODE, "undefined constant DoesntExist"
        @[DoesntExist]
        class Moo
        end
        CODE
    ...
    it "errors if annotation doesn't point to an annotation type" do
      assert_error <<-CODE, "Int32 is not an annotation, it's a struct"
        @[Int32]
        class Moo
        end
        CODE
    ...
    it "errors if using annotation other than ThreadLocal for class vars" do
      assert_error <<-CODE, "class variables can only be annotated with ThreadLocal"
        annotation Foo
        end
    ...
    it "errors if using invalid annotation on fun" do
      assert_error <<-CODE, "funs can only be annotated with: NoInline, AlwaysInline, Naked, ReturnsTwice, Raises, CallConvention"
        annotation Foo
        end
  ...
  it "errors when annotate instance variable in subclass" do
    assert_error <<-CODE, "can't annotate @x in Child because it was first defined in Base"
      annotation Foo
      end
  ...
  it "errors if wanting to add type inside annotation (1) (#8614)" do
    assert_error <<-CODE, "can't declare type inside annotation Ann"
      annotation Ann
      end
