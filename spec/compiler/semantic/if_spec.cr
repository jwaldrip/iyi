...
describe "Semantic: if" do
  ...
  it "restricts type with !var and ||" do
    assert_type(<<-CODE, inject_primitives: true) { union_of bool, int32 }
      a = 1 == 1 ? 1 : nil
      !a || a + 2
      CODE
  end
  ...
  it "restricts and doesn't unify union types" do
    assert_type(<<-CODE) { union_of(nil_type, int32) }
      class Foo
      end
  ...
  it "restricts && of !var.is_a(...)" do
    assert_type(<<-CODE) { union_of int32, float64, bool }
      def foo
        a = 1 || 2.0 || 'c'
  ...
  it "includes pointer types in falsey branch" do
    assert_type(<<-CODE) { nilable union_of bool, pointer_of(int32), int32 }
      def foo
        x = 1
  ...
  it "doesn't fail on Expressions condition (1)" do
    assert_type(<<-CODE) { nilable int32.metaclass }
      def foo
        if (v = 1; true)
          typeof(v)
        end
  ...
  it "doesn't fail on Expressions condition (2)" do
    assert_type(<<-CODE) { nilable nil_type.metaclass }
      def foo
        if (v = nil; true)
          typeof(v)
        end
