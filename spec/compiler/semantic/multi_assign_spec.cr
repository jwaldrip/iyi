...
describe "Semantic: multi assign" do
  context "without strict_multi_assign" do
    ...
    it "errors if assigning non-Indexable to splat (#11414)" do
      assert_error <<-CODE, "right-hand side of one-to-many assignment must be an Indexable, not Foo"
        require "prelude"
  ...
  context "strict_multi_assign" do
    it "errors if assigning tuple to fewer targets" do
      assert_error <<-CODE, "cannot assign Tuple(Int32, Int32, String) to 2 targets", flags: "strict_multi_assign"
        require "prelude"
    ...
    pending "errors if assigning tuple to more targets" do
      assert_error <<-CODE, "cannot assign Tuple(Int32) to 2 targets", flags: "strict_multi_assign"
        require "prelude"
    ...
    it "errors if assigning union of tuples to fewer targets" do
      assert_error <<-CODE, "cannot assign (Tuple(Int32, Int32, Int32) | Tuple(Int32, Int32, Int32, Int32)) to 2 targets", flags: "strict_multi_assign"
        require "prelude"
    ...
    it "doesn't error if some type in union matches target count" do
      assert_type(<<-CODE, flags: "strict_multi_assign") { tuple_of [int32, union_of(int32, string)] }
        require "prelude"
    ...
    it "errors if assigning non-Indexable (#11414)" do
      assert_error <<-CODE, "right-hand side of one-to-many assignment must be an Indexable, not Foo", flags: "strict_multi_assign"
        require "prelude"
