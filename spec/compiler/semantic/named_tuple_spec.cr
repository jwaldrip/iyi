...
describe "Semantic: named tuples" do
  ...
  it "doesn't match in type restriction" do
    assert_error <<-CODE, "expected argument #1 to 'foo' to be NamedTuple(x: Int32, y: Int32), not NamedTuple(x: Int32, y: Char)"
  ...
  it "doesn't match type restriction with instance" do
    assert_error <<-CODE, "expected argument #1 to 'Foo(NamedTuple(a: Int32)).foo' to be NamedTuple(a: Int32), not NamedTuple(a: Float64)"
      class Foo(T)
        def self.foo(x : T)
        end
      end
  ...
  it "can assign to union of compatible named tuple" do
    assert_type(<<-CODE, inject_primitives: true) { union_of(named_tuple_of({"x": int32}), named_tuple_of({"x": int32, "y": string})) }
      tup1 = {x: 1, y: "foo"}
      tup2 = {x: 3}
      tup3 = {y: "bar", x: 2}
  ...
  it "allows tuple covariance" do
    assert_type(<<-CODE) { named_tuple_of({"foo": types["Foo"].virtual_type!}) }
      class Obj
        def initialize
          @tuple = {foo: Foo.new}
        end
  ...
  it "merges two named tuple with same keys but different types" do
    assert_type(<<-CODE, inject_primitives: true) { named_tuple_of({"x": union_of(char, string), "y": nilable(int32)}) }
      def foo
        if 1 == 2
          {x: "foo", y: 1}
        else
  ...
  it "accept named tuple in type restriction" do
    assert_type(<<-CODE) { named_tuple_of({"foo": types["Bar"]}) }
      class Foo
      end
  ...
  it "accepts named tuple covariance in array" do
    assert_type(<<-CODE) { named_tuple_of({"x": types["Foo"].virtual_type!, "y": types["Foo"].virtual_type!}) }
      require "prelude"
