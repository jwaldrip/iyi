...
describe "Semantic: nilable instance var" do
  it "says instance var was not initialized in all of the initialize methods" do
    assert_error <<-CODE, "this 'initialize' doesn't explicitly initialize instance variable '@foo' of Foo, rendering it nilable"
      class Foo
        def initialize
          @foo = 1
        end
  ...
  it "says instance var was not initialized in all of the initialize methods (2)" do
    assert_error <<-CODE, "can't infer the type of instance variable '@foo' of Foo", inject_primitives: true
      abstract class Foo
        def foo
          @foo
        end
  ...
  it "says instance var was not initialized in all of the initialize methods, with var declaration" do
    assert_error <<-CODE, "instance variable '@foo' of Foo was not initialized directly in all of the 'initialize' methods, rendering it nilable. Indirect initialization is not supported."
      class Foo
        @foo : Int32
  ...
  it "says instance var was used before initialized" do
    assert_error <<-CODE, "Instance variable '@foo' was used before it was initialized in one of the 'initialize' methods, rendering it nilable"
      class Foo
        def initialize
          @foo
          @foo = 1
  ...
  it "finds type that doesn't initialize instance var (#1222)" do
    assert_error <<-CODE, "this 'initialize' doesn't initialize instance variable '@x' of Base, with Unreferenced < Base, rendering it nilable"
      class Base
        def initialize
          @x = 0
        end
