...
describe "Semantic: proc" do
  ...
  it "types proc pointer to instance method" do
    assert_type(<<-CODE) { proc_of(int32) }
      class Foo
        def initialize
          @x = 1
        end
  ...
  it "binds proc literal to arguments and body" do
    assert_type(<<-CODE) { proc_of(union_of(int32, char)) }
      x = 1
      f = -> { x }
      x = 'a'
      f
  ...
  it "has proc literal as restriction and errors if output is different" do
    assert_error <<-CODE, "expected argument #1 to 'foo' to be Proc(Int32, Float64), not Proc(Int32, Int32)"
      def foo(x : Int32 -> Float64)
        x.call(1)
      end
  ...
  it "has proc literal as restriction and errors if input is different" do
    assert_error <<-CODE, "expected argument #1 to 'foo' to be Proc(Int32, Float64), not Proc(Int64, Float64)", inject_primitives: true
      def foo(x : Int32 -> Float64)
        x.call(1)
      end
  ...
  it "has proc literal as restriction and errors if sizes are different" do
    assert_error <<-CODE, "expected argument #1 to 'foo' to be Proc(Int32, Float64), not Proc(Int32, Int32, Float64)", inject_primitives: true
      def foo(x : Int32 -> Float64)
        x.call(1)
      end
  ...
  it "disallows casting a proc type to one accepting more arguments" do
    assert_error(<<-CODE, "can't cast", inject_primitives: true)
      f = ->(x : Int32) { x.to_f }
  ...
  it "allows casting a proc type to one with void argument" do
    assert_type(<<-CODE, inject_primitives: true) { proc_of [int32, void] }
      f = ->(x : Int32) { x.to_f }
      f.as(Int32 -> Void)
      CODE
  end
  ...
  it "disallows casting a proc type to one accepting less arguments" do
    assert_error <<-CODE, "can't cast Proc(Int32, Float64) to Proc(Float64)", inject_primitives: true
      f = ->(x : Int32) { x.to_f }
      f.as(-> Float64)
      CODE
  end
  ...
  it "disallows casting a proc type to one accepting same size argument but different output" do
    assert_error <<-CODE, "can't cast Proc(Int32, Float64) to Proc(Int32, Int32)", inject_primitives: true
      f = ->(x : Int32) { x.to_f }
      f.as(Int32 -> Int32)
      CODE
  end
  ...
  it "disallows casting a proc type to one accepting same size argument but different input" do
    assert_error <<-CODE, "can't cast Proc(Int32, Float64) to Proc(Float64, Float64)", inject_primitives: true
      f = ->(x : Int32) { x.to_f }
  ...
  it "types proc literal hard type inference (1)" do
    assert_type(<<-CODE) { proc_of(types["Foo"], tuple_of([types["Foo"], int32])) }
      class Foo
  ...
  it "allows passing NoReturn type for any return type (1)" do
    assert_type(<<-CODE, inject_primitives: true) { no_return }
      lib LibC
        fun exit : NoReturn
      end
  ...
  it "allows new on proc type with less block params" do
    assert_type(<<-CODE) { proc_of(int32, int32) }
      #{proc_new}
  ...
  it "says wrong number of block params in new on proc type" do
    assert_error <<-CODE, "wrong number of block parameters (given 2, expected 1)"
      #{proc_new}
  ...
  it "allows writing a function type with Proc" do
    assert_type(<<-CODE) { proc_of(int32, int32).metaclass }
      Proc(Int32, Int32)
      CODE
  end
  ...
  it "allows using Proc as restriction (3)" do
    assert_type(<<-CODE, inject_primitives: true) { int32.metaclass }
  ...
  it "doesn't need to deduce type of block if return is void" do
    assert_type(<<-CODE) { proc_of(types["Foo"], nil_type) }
      class Foo
        def initialize
          @bar = 1
        end
  ...
  it "gives correct error message when proc return type is incorrect (#219)" do
    assert_error <<-CODE, "argument 'f' of 'LibFoo#bar' must be a Proc returning Int32, not Float64"
      lib LibFoo
  ...
  it "types proc literal with a type that was never instantiated" do
    assert_type(<<-CODE) { proc_of(types["Foo"], int32) }
      class Foo
        def initialize(@x : Int32)
        end
  ...
  it "types proc pointer with a type that was never instantiated" do
    assert_type(<<-CODE) { proc_of(types["Foo"], types["Foo"]) }
      class Foo
        def initialize(@x : Int32)
        end
  ...
  it "uses array argument of proc arg (2)" do
    assert_type(<<-CODE) { proc_of(array_of(types["Foo"].virtual_type), types["Foo"].virtual_type) }
      require "prelude"
  ...
  it "gets pointer to lib fun without specifying types" do
    assert_type(<<-CODE) { proc_of(int32, float64) }
      lib LibFoo
  ...
  it "gets pointer to lib fun with compatible parameter types (1)" do
    assert_type(<<-CODE) { proc_of(proc_of(float64), int32) }
      lib LibFoo
        fun foo(x : ->) : Int32
      end
  ...
  it "gets pointer to lib fun with compatible parameter types (2)" do
    assert_type(<<-CODE) { proc_of(pointer_of(float64), int32) }
      lib LibFoo
  ...
  it "gets pointer to lib fun with compatible `#to_unsafe` type" do
    assert_type(<<-CODE) { proc_of(types["Foo"], float64) }
      lib LibFoo
        fun foo(x : Int32) : Float64
      end
  ...
  it "gets pointer to variadic lib fun" do
    assert_type(<<-CODE) { proc_of([int32, int16, int8, float64] of Type) }
      lib LibFoo
        fun foo(x : Int32, ...) : Float64
      end
  ...
  it "errors if pointer to lib fun has incompatible parameter type" do
    assert_error <<-CODE, "argument 'x' of 'LibFoo#foo' must be Int32, not Foo (nor Char returned by 'Foo#to_unsafe')"
      lib LibFoo
        fun foo(x : Int32)
      end
  ...
  it "errors if pointer to lib fun has incorrect number of parameters" do
    assert_error <<-CODE, "wrong number of arguments for 'LibFoo#foo' (given 1, expected 2)"
      lib LibFoo
  ...
  %w(Object Value Reference Number Int Float Struct Class Proc Tuple Enum StaticArray Pointer).each do |type|
    it "disallows #{type} in procs" do
      assert_error <<-CODE, "can't use #{type} as a Proc argument type"
        ->(x : #{type}) { }
        CODE
    end
  ...
  it "allows metaclass in procs" do
    assert_type(<<-CODE) { proc_of(types["Foo"].metaclass, types["Foo"]) }
      class Foo
      end
  ...
  it "allows metaclass in proc return types" do
    assert_type(<<-CODE) { proc_of(types["Foo"].metaclass) }
      class Foo
      end
  ...
  it "allows metaclass in proc notation parameter type" do
    assert_type(<<-CODE) { proc_of(types["Foo"].metaclass, nil_type) }
      class Foo
      end
  ...
  it "sets proc type as void if explicitly told so, when using new" do
    assert_type(<<-CODE) { proc_of(int32, nil_type) }
      #{proc_new}
  ...
  it "accesses T and R" do
    assert_type(<<-CODE) { tuple_of([tuple_of([int32]).metaclass, char.metaclass]) }
      struct Proc
        def t
          {T, R}
        end
  ...
  it "can match *T in block argument" do
    assert_type(<<-CODE) { bool.metaclass }
      struct Tuple
  ...
  it "says wrong number of arguments" do
    assert_error <<-CODE, "no overload matches", inject_primitives: true
      ->(x : Int32) { }.call
      CODE
  end
  ...
  it "declares an instance variable with splat in proc notation" do
    assert_type(<<-CODE) { proc_of([int32, char, string]) }
      class Foo
        @x : *{Int32, Char} -> String
  ...
  it "*doesn't* merge Proc that returns Nil with another one that returns something else (#3655) (this was reverted)" do
    assert_type(<<-CODE) { union_of proc_of(int32, int32), proc_of(int32, nil_type) }
      a = ->(x : Int32) { 1 }
      b = ->(x : Int32) { nil }
      a || b
      CODE
  ...
  it "*doesn't* merge Proc that returns NoReturn with another one that returns something else (#9971)" do
    assert_type(<<-CODE) { union_of proc_of(int32, int32), proc_of(int32, no_return) }
      lib LibC
        fun exit : NoReturn
      end
  ...
  it "errors when using macro as proc value (top-level) (#7465)" do
    ex = assert_error <<-CODE, "undefined method 'bar'"
           macro bar
           end
  ...
  it "errors when using macro as proc value (top-level with obj) (#7465)" do
    ex = assert_error <<-CODE, "undefined method 'bar' for Foo.class"
           class Foo
             macro bar
             end
           end
  ...
  it "virtualizes proc type (#6789)" do
    assert_type(<<-CODE) { proc_of(types["Foo"].virtual_type!, types["Foo"].virtual_type!) }
      class Foo
      end
  ...
  it "can pass Proc(X, T) to Proc(X, Nil) in type restriction (#8964)" do
    assert_type(<<-CODE) { proc_of string, nil_type }
  ...
  it "doesn't cause upcast bug (#8428)" do
    assert_type(<<-CODE) { union_of proc_of(string), proc_of(nil_type), nil_type }
      def foo
        if true
          begin
            ->{""}
