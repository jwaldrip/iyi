...
describe "Semantic: recursive struct check" do
  it "errors on recursive struct" do
    ex = assert_error <<-CODE, "recursive struct Test detected"
           struct Test
  ...
  it "errors on recursive struct inside module" do
    ex = assert_error <<-CODE, "recursive struct Foo::Test detected"
           struct Foo::Test
  ...
  it "errors on recursive generic struct inside module" do
    ex = assert_error <<-CODE, "recursive struct Foo::Test(T) detected"
           struct Foo::Test(T)
  ...
  it "errors on mutually recursive struct" do
    ex = assert_error <<-CODE, "recursive struct Foo detected"
           struct Foo
  ...
  it "detects recursive generic struct through module (#4720)" do
    ex = assert_error <<-CODE, "recursive struct Foo(T) detected"
           module Bar
           end
  ...
  it "detects recursive struct through inheritance (#3071)" do
    ex = assert_error <<-CODE, "recursive struct Bar detected"
           abstract struct Foo
           end
  ...
  it "errors on recursive struct through recursive alias (#4454) (#4455)" do
    ex = assert_error <<-CODE, "recursive struct Foo detected (recursive aliases are structs)"
           struct Bar(T)
             def initialize(@x : T)
             end
           end
  ...
  it "errors on private recursive type" do
    assert_error <<-CODE, "recursive struct Test detected"
      private struct Test
        def initialize(@test : Test?)
        end
      end
