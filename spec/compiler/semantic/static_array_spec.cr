...
describe "Semantic: static array" do
  ...
  it "types static array with type as size" do
    assert_type(<<-CODE) { static_array_of(char, 1) }
      class Foo(N)
        def self.foo
  ...
  it "errors if trying to instantiate static array with N not an integer" do
    assert_error <<-CODE, "can't instantiate StaticArray(T, N) with N = Int32 (N must be an integer)"
  ...
  it "errors on negative static array size" do
    assert_error <<-CODE, "can't instantiate StaticArray(T, N) with N = -1 (N must be positive)"
      x = uninitialized Int32[-1]
      CODE
  end

  it "types static array new with size being a constant" do
    assert_type(<<-CODE) { static_array_of(char, 3) }
      SIZE = 3
  ...
  it "types static array new with size being a computed constant" do
    assert_type(<<-CODE, inject_primitives: true) { static_array_of(char, 100) }
      struct Int
        def //(other : Int)
          0
        end
  ...
  it "doesn't match other number type argument of static array (#1203)" do
    assert_error <<-CODE, "expected argument #1 to 'fn' to be StaticArray(Int32, 11), not StaticArray(Int32, 10)"
  ...
  it "doesn't crash on sizeof (#8858)" do
    assert_error <<-CODE, "can't use sizeof(Int32) as a generic type argument"
      alias BadArray = Int32[sizeof(Int32)]
      CODE
  end

  it "doesn't crash on instance_sizeof (#8858)" do
    assert_error <<-CODE, "can't use instance_sizeof(String) as a generic type argument"
      alias BadArray = Int32[instance_sizeof(String)]
      CODE
  end

  it "doesn't crash on offsetof (#8858)" do
    assert_error <<-CODE, "can't use offsetof(Foo, @foo) as a generic type argument"
      class Foo
        @foo : Int32 = 0
      end
