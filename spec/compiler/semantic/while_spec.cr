...
describe "Semantic: while" do
  ...
  it "types while with multiple breaks with value" do
    assert_type(<<-CODE) { nilable union_of(char, tuple_of([string, int32])) }
      while 1
        break 'a' if 1
        break "", 123 if 1
      end
  ...
  it "types endless while with multiple breaks with value" do
    assert_type(<<-CODE) { union_of(char, tuple_of([string, int32])) }
      while true
        break 'a' if 1
        break "", 123 if 1
      end
  ...
  it "uses type at end of endless while if variable is reassigned, but not before first break" do
    assert_type(<<-CODE, inject_primitives: true) { nilable union_of(int32, char) }
      while true
        if 1 == 1
          break
        end
  ...
  it "uses type at end of endless while if variable is reassigned, but not before first break (2)" do
    assert_type(<<-CODE, inject_primitives: true) { union_of(int32, char, string) }
      a = ""
      while true
        if 1 == 1
          break
  ...
  it "rebinds condition variable after while body (#6158)" do
    assert_type(<<-CODE, inject_primitives: true) { nilable types["Foo"] }
      class Foo
        @parent : self?
  ...
  it "finds all while cond assign targets in expressions (6)" do
    assert_type(<<-CODE, inject_primitives: true) { tuple_of [int32, int32] }
  ...
  it "doesn't fail on Expressions condition (1)" do
    assert_type(<<-CODE) { union_of int32.metaclass, char }
      def foo
        while (v = 1; true)
          return typeof(v)
        end
  ...
  it "doesn't fail on Expressions condition (2)" do
    assert_type(<<-CODE) { union_of nil_type.metaclass, char }
      def foo
        while (v = nil; true)
          return typeof(v)
        end
