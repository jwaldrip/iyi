...
module Iyi
  ...
  class MainVisitor < SemanticVisitor
    ...
    def visit(node : ExceptionHandler)
      ...
      # cleanup that matters; `defer` runs on both.
      #
      # The condition is iyi's *library*, not the file's extension. When
      # this refusal was written a `.cr` file always meant Crystal's library
      # and Crystal's runtime, where `rescue` works and is kept. A `.cr`
      # file on iyi's prelude is the case that did not exist then, and it is
      # the worst of the three: it compiles, nothing raises a catchable
      # exception because iyi's `raise` panics and iyi has no `Exception` at
      # all, and the handler is silently dead. Measured: a two-line program
      # with `begin raise "boom" rescue puts "caught" end` built clean and
      # printed the panic, never "caught".
      if @program.iyi_prelude? && !node.iyi_defer?
        ...
        if node.rescues || node.else
          if iyi_file
            ...
            node.raise "this `rescue` cannot run: the program is built on iyi's library, where `raise` panics and " \
                       "there is no `Exception` to catch. Crystal's exceptions come with Crystal's runtime - build " \
                       "with `--crystal` - and iyi's answer is an error returned as a value (SPEC.md III.1)"
          end
        elsif node.ensure
          if iyi_file
            ...
            node.raise "this `ensure` is not reliable on iyi's library: a panic unwinds by registry and skips it " \
                       "(SPEC.md III.1.4). Build with `--crystal` for Crystal's runtime, or write `defer`"
          end
        end
      end
